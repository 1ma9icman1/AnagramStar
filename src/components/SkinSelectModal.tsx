  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-gb-pop">
      <div
        ...
        className={`w-full max-w-lg border-2 p-4 sm:p-6 flex flex-col shadow-2xl transition-all duration-300 ${
          isGb
            ? "border-[#0f380f] bg-[#9bbc0f] text-[#0f380f] font-['Press_Start_2P',monospace] shadow-[6px_6px_0_#0f380f]"
            : "border-[#00ff66] bg-[#050e08]/95 text-emerald-100 font-mono shadow-[0_0_30px_rgba(0,255,102,0.25)]"
        }`}
      >
        {/* Header */}
        <div
          className={`flex items-center justify-between pb-3 border-b-2 ${
            isGb ? 'border-[#0f380f]' : 'border-emerald-500/40'
          }`}
        >
          <div className="flex items-center gap-2">
            <span className="text-base sm:text-lg">{isGb ? '👾' : '⚡'}</span>
            <div>
              <h2 className="text-xs sm:text-sm font-bold tracking-wider">
                {isGb ? 'SELECT INTERFACE' : 'INTERFACE SELECTION MATRIX'}
              </h2>
              <p
                className={`text-[8px] sm:text-[9px] mt-0.5 ${
                  isGb ? 'text-[#306230]' : 'text-emerald-400/80'
                }`}
              >
                Choose your preferred visual & audio experience
              </p>
            </div>
          </div>
          {canClose && onClose && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 my-4">
          ...
          <button
            ...
            className={`p-3 sm:p-4 border-2 text-left flex flex-col justify-between transition-all cursor-pointer relative overflow-hidden ${
          ...
          >
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <div className="font-bold text-xs">GAME BOY DMG</div>
                  <div className="text-[7px] opacity-75">1989 Retro Console</div>
                </div>
              </div>

              {/* Visual preview widget */}
              <div className="my-2 p-2 border border-[#0f380f] bg-[#9bbc0f] text-[#0f380f] text-[7px] space-y-1">
                <div className="flex justify-between items-center border-b border-[#0f380f]/30 pb-0.5">
                  <span>DOT MATRIX LCD</span>
                  <span className="text-[#306230]">● BATTERY</span>
                </div>
                <div className="flex gap-1 justify-center py-1">
                  {['W', 'O', 'R', 'D'].map((char, i) => (
                    <span
                      key={i}
                      className="px-1.5 py-0.5 border border-[#0f380f] bg-[#8bac0f] font-bold text-[8px]"
                    >
                      {char}
                    </span>
                  ))}
              ...
              <ul className="text-[7px] space-y-1 mt-2 opacity-90">
                <li>• Authentic DMG-01 console chassis</li>
                <li>• Physical D-Pad & A/B action buttons</li>
                <li>• 4 Dot-Matrix LCD color palettes</li>
                <li>• 8-bit pulse wave chiptune audio</li>
              </ul>
            </div>
          </button>
          ...
          >
            ...
            <div>
              ...
              {/* Visual preview widget */}
              <div className="my-2 p-2 border border-[#00ff66]/50 bg-black/60 text-[#00ff66] text-[7px] space-y-1">
                <div className="flex justify-between items-center border-b border-emerald-800/40 pb-0.5">
                  <span>NEON CYBER DECK</span>
                ...
                <div className="flex gap-1 justify-center py-1">
                  {['H', 'A', 'C', 'K'].map((char, i) => (
                    <span
                      key={i}
                      className="px-1.5 py-0.5 border border-[#00ff66] bg-[#00ff66]/20 font-bold text-[8px] text-white shadow-[0_0_6px_#00ff66]"
                    >
                      {char}
                    </span>
                  ))}
              ...
              <ul className="text-[7px] space-y-1 mt-2 text-emerald-300/90">
                <li>• Fullscreen dynamic digital rain</li>
                <li>• Glowing holographic letter tiles</li>
                <li>• High-tech cyberpunk telemetry HUD</li>
                <li>• Direct drag, touch & keyboard control</li>
              </ul>
            </div>
          </button>
        </div>
        ...
        <label className="flex items-center gap-2 text-[8px] cursor-pointer py-1 select-none">
          ...
          />
          <span className={isGb ? 'text-[#306230]' : 'text-emerald-400'}>
            Remember preference for future launches (switch anytime in header/settings)
          </span>
        </label>
        ...
        <div className="pt-3 mt-2 border-t-2 border-current/20 flex gap-2">
          ...
          >
            ► ENTER WITH {selectedSkin === 'gameboy' ? 'GAME BOY SKIN' : 'ORIGINAL CYBER SKIN'}
          </button>
        </div>
      </div>
    </div>
